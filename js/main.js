  ...
  const addHighlightTool = function () {
    ...
    if (isPrismjs) {
      $figureHighlight.forEach(item => {
        if (highlightLang) {
          const langName = item.getAttribute('_data-language') || 'Code'
  ...
  const runJustifiedGallery = function (ele) {
    ...
    const runJustifiedGallery = (item, arr) => {
      if (!item.classList.contains('lazyload')) item.innerHTML = htmlStr(arr)
        ...
        const limit = item.getAttribute('_data-limit')
 ...
 */
  ...
  const addRuntime = () => {
    ...
    if ($runtimeCount) {
      const publishDate = $runtimeCount.getAttribute('_data-publishDate')
  ...
  const addLastPushDate = () => {
    ...
    if ($lastPushDateItem) {
      const lastPushDate = $lastPushDateItem.getAttribute('_data-lastPushDate')
      ...
      document.querySelectorAll('#article-container .tab > button').forEach(function (item) {
        item.addEventListener('click', function (e) {
          ...
          if (!$tabItem.classList.contains('active')) {
            ...
            const tabId = $this.getAttribute('_data-href').replace('#', '')
